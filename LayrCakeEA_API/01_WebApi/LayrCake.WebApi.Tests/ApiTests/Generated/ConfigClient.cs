...
{
    public partial class Api_Config_Tests : BaseTestInitialise
    {
		public async Task<List<Config>> GetAll(HttpClient client = null)
		{
	        var request = client.GetAsync("tables/config");
            ...
            var returnItems = JsonConvert.DeserializeObject<List<Config>>(stream);
		...
		public async Task Insert(Config insertItem, HttpClient client = null)
        {
            client = client ?? testClient;
			Uri location;

            // Post a new entity and verify entity returned
            HttpResponseMessage postResponse = client.PostAsJsonAsync("tables/config", insertItem).Result;
            Assert.AreEqual(HttpStatusCode.Created, postResponse.StatusCode);
            location = postResponse.Headers.Location;
            Assert.IsNotNull(location);

            Config resultPost = postResponse.Content.ReadAsAsync<Config>().Result;
            ...
            Config resultRead = queryResponse.Content.ReadAsAsync<Config>().Result;
            //this.VerifySimpleEntitiesEqual(entity, result);
