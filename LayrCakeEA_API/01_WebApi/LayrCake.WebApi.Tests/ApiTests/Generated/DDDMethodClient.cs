...
{
    public partial class Api_DDDMethod_Tests : BaseTestInitialise
    {
		public async Task<List<DDDMethod>> GetAll(HttpClient client = null)
		{
	        var request = client.GetAsync("tables/dddmethod");
            ...
            var returnItems = JsonConvert.DeserializeObject<List<DDDMethod>>(stream);
		...
		public async Task Insert(DDDMethod insertItem, HttpClient client = null)
        {
            client = client ?? testClient;
			Uri location;

            // Post a new entity and verify entity returned
            HttpResponseMessage postResponse = client.PostAsJsonAsync("tables/dddmethod", insertItem).Result;
            Assert.AreEqual(HttpStatusCode.Created, postResponse.StatusCode);
            location = postResponse.Headers.Location;
            Assert.IsNotNull(location);

            DDDMethod resultPost = postResponse.Content.ReadAsAsync<DDDMethod>().Result;
            ...
            DDDMethod resultRead = queryResponse.Content.ReadAsAsync<DDDMethod>().Result;
            //this.VerifySimpleEntitiesEqual(entity, result);
