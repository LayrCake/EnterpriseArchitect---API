...
{
    public partial class Api_DDDProblemDomain_Tests : BaseTestInitialise
    {
		public async Task<List<DDDProblemDomain>> GetAll(HttpClient client = null)
		{
	        var request = client.GetAsync("tables/dddproblemdomain");
            ...
            var returnItems = JsonConvert.DeserializeObject<List<DDDProblemDomain>>(stream);
		...
		public async Task Insert(DDDProblemDomain insertItem, HttpClient client = null)
        {
            client = client ?? testClient;
			Uri location;

            // Post a new entity and verify entity returned
            HttpResponseMessage postResponse = client.PostAsJsonAsync("tables/dddproblemdomain", insertItem).Result;
            Assert.AreEqual(HttpStatusCode.Created, postResponse.StatusCode);
            location = postResponse.Headers.Location;
            Assert.IsNotNull(location);

            DDDProblemDomain resultPost = postResponse.Content.ReadAsAsync<DDDProblemDomain>().Result;
            ...
            DDDProblemDomain resultRead = queryResponse.Content.ReadAsAsync<DDDProblemDomain>().Result;
            //this.VerifySimpleEntitiesEqual(entity, result);
