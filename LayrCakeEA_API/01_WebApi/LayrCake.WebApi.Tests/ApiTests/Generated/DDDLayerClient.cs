...
{
    public partial class Api_DDDLayer_Tests : BaseTestInitialise
    {
		public async Task<List<DDDLayer>> GetAll(HttpClient client = null)
		{
	        var request = client.GetAsync("tables/dddlayer");
            ...
            var returnItems = JsonConvert.DeserializeObject<List<DDDLayer>>(stream);
		...
		public async Task Insert(DDDLayer insertItem, HttpClient client = null)
        {
            client = client ?? testClient;
			Uri location;

            // Post a new entity and verify entity returned
            HttpResponseMessage postResponse = client.PostAsJsonAsync("tables/dddlayer", insertItem).Result;
            Assert.AreEqual(HttpStatusCode.Created, postResponse.StatusCode);
            location = postResponse.Headers.Location;
            Assert.IsNotNull(location);

            DDDLayer resultPost = postResponse.Content.ReadAsAsync<DDDLayer>().Result;
            ...
            DDDLayer resultRead = queryResponse.Content.ReadAsAsync<DDDLayer>().Result;
            //this.VerifySimpleEntitiesEqual(entity, result);
