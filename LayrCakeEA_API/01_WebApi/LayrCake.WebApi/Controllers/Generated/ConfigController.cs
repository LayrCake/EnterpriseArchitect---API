...
namespace LayrCake.WebApi.Controllers.Implementation
{
	[Authorize]
	[MobileAppController]
	[RoutePrefix("tables/config")]
	public partial class ConfigController : ApiController //: AzureTableController<Config>
	{
		internal IConfigRepository _configRepository;
		internal ITraceWriter traceWriter;
		internal Specification<Config> baseSpec = new Specification<Config>(x => true);
        internal ConfigVwmCriteria criteria = new ConfigVwmCriteria();

		public ConfigController(IConfigRepository repository)
		{
			_configRepository = repository ?? new ConfigRepository();
		}
		
        // GET tables/Config
        [HttpGet]
        [Route("")]
		public IQueryable<Config> GetAllConfigs(ODataQueryOptions query)
        { // Temp use Vwm Name Until alternative option can be found to convert Expression (use method in WCF Action Service layer
		    try
            {
				if (query != null)
				{
                    ...
                    {
						var expression = new Visitor().Visit(query.Filter.ToExpression<Config>());
						expression = expression != null ? LambdaExpressionHelper<ConfigVwm, Config>.Convert((Expression<Func<Config, bool>>)expression, typeof(ConfigVwm)) : null;
						criteria.Specification = new Specification<ConfigVwm>((Expression<Func<ConfigVwm, bool>>)expression);
                    }
                    var top = 1000;
                ...
                foreach (var returnItem in _configRepository.GetListQuery(criteria))
        ...
        [HttpGet]
        [Route("{id}", Name = "GetConfigId")]
        public Config GetConfig(string id)
        {
            try
            {
                var specPackageRef = new Specification<ConfigVwm>(i => i.MobileId == id);
                var criteria = new ConfigVwmCriteria { Specification = specPackageRef };

                var result = _configRepository.Get(criteria);
        ...
        // GET tables/Challenge/48D68C86-6EA6-4C25-AA33-223FC9A27959
        [HttpGet]
        [Route("{id:int}", Name="GetConfigId")]
		public Config GetConfig(int id)
        {
		    try
            {
				var result = _configRepository.Get(id);
				return Mapper.FromBusinessObject(result);
            }
            catch (HttpResponseException ex)
            {
		...
		*/
        ...
        [HttpPatch]
        [Route("{id}")]
		public Config PatchConfig(string id, Config item)
        {
		    if (item == null)
            ...
            try
            {
				var result = _configRepository.Update(Mapper.ToBusinessObject(item));
				return Mapper.FromBusinessObject(result);
            }
            catch (HttpResponseException ex)
            {
        ...
        public async Task<IHttpActionResult> PostConfig(Config item)
        {
            ...
            try
            {
				var insert = _configRepository.Insert(Mapper.ToBusinessObject(item));
                var current = Mapper.FromBusinessObject(insert);
				return CreatedAtRoute("GetConfigId", new { id = current.Id }, current);

			    //var insert = Task.Run(() => _configRepository.Insert(Mapper.ToBusinessObject(item)));
                //var mapping = insert.ContinueWith(map => Mapper.FromBusinessObject(map.Result));
                //return CreatedAtRoute("Tables", new { controller = "config", id = mapping.Result.Id }, mapping.Result);
            }
            catch (HttpResponseException ex)
            {
                this.traceWriter.Error(ex, this.Request, LogCategories.TableControllers);
		...
		[HttpDelete]
        [Route("{id:int}")]
		public async Task<IHttpActionResult> DeleteConfig(int id)
        {
			bool result = false;
            try
            {
				var item = _configRepository.Delete(id);
		...
		[HttpDelete]
        [Route("")]
		public async Task<IHttpActionResult> DeleteConfig(Config item)
        {
			bool result = false;
            try
            {
				var returnItem = _configRepository.Delete(Mapper.ToBusinessObject(item));
