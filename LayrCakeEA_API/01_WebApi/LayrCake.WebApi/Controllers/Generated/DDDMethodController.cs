...
namespace LayrCake.WebApi.Controllers.Implementation
{
	[Authorize]
	[MobileAppController]
	[RoutePrefix("tables/dddmethod")]
	public partial class DDDMethodController : ApiController //: AzureTableController<DDDMethod>
	{
		internal IDDDMethodRepository _dDDMethodRepository;
		internal ITraceWriter traceWriter;
		internal Specification<DDDMethod> baseSpec = new Specification<DDDMethod>(x => true);
        internal DDDMethodVwmCriteria criteria = new DDDMethodVwmCriteria();

		public DDDMethodController(IDDDMethodRepository repository)
		{
			_dDDMethodRepository = repository ?? new DDDMethodRepository();
		}
		
        // GET tables/DDDMethod
        [HttpGet]
        [Route("")]
		public IQueryable<DDDMethod> GetAllDDDMethods(ODataQueryOptions query)
        { // Temp use Vwm Name Until alternative option can be found to convert Expression (use method in WCF Action Service layer
		    try
            {
				if (query != null)
				{
                    ...
                    {
						var expression = new Visitor().Visit(query.Filter.ToExpression<DDDMethod>());
						expression = expression != null ? LambdaExpressionHelper<DDDMethodVwm, DDDMethod>.Convert((Expression<Func<DDDMethod, bool>>)expression, typeof(DDDMethodVwm)) : null;
						criteria.Specification = new Specification<DDDMethodVwm>((Expression<Func<DDDMethodVwm, bool>>)expression);
                    }
                    var top = 1000;
                ...
                // Before Fetch
                var result = new List<DDDMethod>();
                foreach (var returnItem in _dDDMethodRepository.GetListQuery(criteria))
        ...
        [HttpGet]
        [Route("{id}", Name = "GetDDDMethodId")]
        public DDDMethod GetDDDMethod(string id)
        {
            try
            {
                var specPackageRef = new Specification<DDDMethodVwm>(i => i.MobileId == id);
                var criteria = new DDDMethodVwmCriteria { Specification = specPackageRef };

                var result = _dDDMethodRepository.Get(criteria);
        ...
        // GET tables/Challenge/48D68C86-6EA6-4C25-AA33-223FC9A27959
        [HttpGet]
        [Route("{id:int}", Name="GetDDDMethodId")]
		public DDDMethod GetDDDMethod(int id)
        {
		    try
            {
				var result = _dDDMethodRepository.Get(id);
				return Mapper.FromBusinessObject(result);
            }
            catch (HttpResponseException ex)
            {
		...
		*/
        // PATCH tables/DDDMethod/48D68C86-6EA6-4C25-AA33-223FC9A27959
        [HttpPatch]
        [Route("{id}")]
		public DDDMethod PatchDDDMethod(string id, DDDMethod item)
        {
		    if (item == null)
            ...
            try
            {
				var result = _dDDMethodRepository.Update(Mapper.ToBusinessObject(item));
				return Mapper.FromBusinessObject(result);
            }
            catch (HttpResponseException ex)
            {
		...
		// POST tables/DDDMethod
		[HttpPost]
        [Route("")]
        public async Task<IHttpActionResult> PostDDDMethod(DDDMethod item)
        {
		    if (item == null)
                throw new HttpResponseException(Request.CreateErrorResponse(HttpStatusCode.BadRequest, TResources.TableController_NullRequestBody));
            ...
            try
            {
				var insert = _dDDMethodRepository.Insert(Mapper.ToBusinessObject(item));
                var current = Mapper.FromBusinessObject(insert);
				return CreatedAtRoute("GetDDDMethodId", new { id = current.Id }, current);

			    //var insert = Task.Run(() => _dDDMethodRepository.Insert(Mapper.ToBusinessObject(item)));
                //var mapping = insert.ContinueWith(map => Mapper.FromBusinessObject(map.Result));
                //return CreatedAtRoute("Tables", new { controller = "dddmethod", id = mapping.Result.Id }, mapping.Result);
            }
            catch (HttpResponseException ex)
            {
                this.traceWriter.Error(ex, this.Request, LogCategories.TableControllers);
		...
		[HttpDelete]
        [Route("{id:int}")]
		public async Task<IHttpActionResult> DeleteDDDMethod(int id)
        {
			bool result = false;
            try
            {
				var item = _dDDMethodRepository.Delete(id);
		...
		[HttpDelete]
        [Route("")]
		public async Task<IHttpActionResult> DeleteDDDMethod(DDDMethod item)
        {
			bool result = false;
            try
            {
				var returnItem = _dDDMethodRepository.Delete(Mapper.ToBusinessObject(item));
