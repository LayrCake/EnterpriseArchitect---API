...
namespace LayrCake.WebApi.Controllers.Implementation
{
	[Authorize]
	[MobileAppController]
	[RoutePrefix("tables/dddlayer")]
	public partial class DDDLayerController : ApiController //: AzureTableController<DDDLayer>
	{
		internal IDDDLayerRepository _dDDLayerRepository;
		internal ITraceWriter traceWriter;
		internal Specification<DDDLayer> baseSpec = new Specification<DDDLayer>(x => true);
        internal DDDLayerVwmCriteria criteria = new DDDLayerVwmCriteria();

		public DDDLayerController(IDDDLayerRepository repository)
		{
			_dDDLayerRepository = repository ?? new DDDLayerRepository();
		}
		
        // GET tables/DDDLayer
        [HttpGet]
        [Route("")]
		public IQueryable<DDDLayer> GetAllDDDLayers(ODataQueryOptions query)
        { // Temp use Vwm Name Until alternative option can be found to convert Expression (use method in WCF Action Service layer
		    try
            {
				if (query != null)
				{
                    ...
                    {
						var expression = new Visitor().Visit(query.Filter.ToExpression<DDDLayer>());
						expression = expression != null ? LambdaExpressionHelper<DDDLayerVwm, DDDLayer>.Convert((Expression<Func<DDDLayer, bool>>)expression, typeof(DDDLayerVwm)) : null;
						criteria.Specification = new Specification<DDDLayerVwm>((Expression<Func<DDDLayerVwm, bool>>)expression);
                    }
                    var top = 1000;
                ...
                // Before Fetch
                var result = new List<DDDLayer>();
                foreach (var returnItem in _dDDLayerRepository.GetListQuery(criteria))
        ...
        [HttpGet]
        [Route("{id}", Name = "GetDDDLayerId")]
        public DDDLayer GetDDDLayer(string id)
        {
            try
            {
                var specPackageRef = new Specification<DDDLayerVwm>(i => i.MobileId == id);
                var criteria = new DDDLayerVwmCriteria { Specification = specPackageRef };

                var result = _dDDLayerRepository.Get(criteria);
        ...
        // GET tables/Challenge/48D68C86-6EA6-4C25-AA33-223FC9A27959
        [HttpGet]
        [Route("{id:int}", Name="GetDDDLayerId")]
		public DDDLayer GetDDDLayer(int id)
        {
		    try
            {
				var result = _dDDLayerRepository.Get(id);
				return Mapper.FromBusinessObject(result);
            }
            catch (HttpResponseException ex)
            {
		...
		*/
        // PATCH tables/DDDLayer/48D68C86-6EA6-4C25-AA33-223FC9A27959
        [HttpPatch]
        [Route("{id}")]
		public DDDLayer PatchDDDLayer(string id, DDDLayer item)
        {
		    if (item == null)
            ...
            try
            {
				var result = _dDDLayerRepository.Update(Mapper.ToBusinessObject(item));
				return Mapper.FromBusinessObject(result);
            }
            catch (HttpResponseException ex)
            {
		...
		// POST tables/DDDLayer
		[HttpPost]
        [Route("")]
        public async Task<IHttpActionResult> PostDDDLayer(DDDLayer item)
        {
		    if (item == null)
                throw new HttpResponseException(Request.CreateErrorResponse(HttpStatusCode.BadRequest, TResources.TableController_NullRequestBody));
            ...
            try
            {
				var insert = _dDDLayerRepository.Insert(Mapper.ToBusinessObject(item));
                var current = Mapper.FromBusinessObject(insert);
				return CreatedAtRoute("GetDDDLayerId", new { id = current.Id }, current);

			    //var insert = Task.Run(() => _dDDLayerRepository.Insert(Mapper.ToBusinessObject(item)));
                //var mapping = insert.ContinueWith(map => Mapper.FromBusinessObject(map.Result));
                //return CreatedAtRoute("Tables", new { controller = "dddlayer", id = mapping.Result.Id }, mapping.Result);
            }
            catch (HttpResponseException ex)
            {
                this.traceWriter.Error(ex, this.Request, LogCategories.TableControllers);
		...
		[HttpDelete]
        [Route("{id:int}")]
		public async Task<IHttpActionResult> DeleteDDDLayer(int id)
        {
			bool result = false;
            try
            {
				var item = _dDDLayerRepository.Delete(id);
		...
		[HttpDelete]
        [Route("")]
		public async Task<IHttpActionResult> DeleteDDDLayer(DDDLayer item)
        {
			bool result = false;
            try
            {
				var returnItem = _dDDLayerRepository.Delete(Mapper.ToBusinessObject(item));
